...
export default function UpcomingMoviesPage() {
  ...
  const fetchPage = async (pageToLoad: number) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/upcoming?page=${pageToLoad}`);
      ...
      const message = err instanceof Error ? err.message : "Failed to load upcoming movies.";
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className={styles.page}>
      <header className={styles.header}>
        ...
        <p className={styles.subtitle}>Coming soon to theaters.</p>
      </header>
